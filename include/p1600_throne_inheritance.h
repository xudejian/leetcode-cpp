#ifndef __P1600_THRONE_INHERITANCE_H__
#define __P1600_THRONE_INHERITANCE_H__

#include "header.h"

namespace leetcode {
namespace p1600_throne_inheritance {
...
class ThroneInheritance {
 private:
  string king;
  std::unordered_map<string, std::vector<string>> children;
 ...
 public:
  ...
  vector<string> getInheritanceOrder() {
    std::vector<string> inOrder;

    preOrder(king, inOrder);
    return inOrder;
  }

 private:
  void preOrder(string name, std::vector<string> &inOrder) {
    if (dead.find(name) == dead.end()) {
      inOrder.push_back(name);
    }
    for (auto p : children[name]) {
      preOrder(p, inOrder);
    }
  }
};
...
#endif  // __P1600_THRONE_INHERITANCE_H__
